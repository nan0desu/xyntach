...
$_fh5_glob =& $this->globals["foreach"]['postsloop'];
if ($this->isArray($_fh5_data) === true)
{
	foreach ($_fh5_data as $this->scope['postkey']=>$this->scope['post'])
	{
		$_fh5_glob["first"] = (string) ($_fh5_glob["index"] === 0);
...
{
	foreach ($_fh4_data as $this->scope['postkey2']=>$this->scope['post2'])
	{
/* -- foreach start output */
?>
					<?php if ((isset($this->scope["post2"]["parentid"]) ? $this->scope["post2"]["parentid"]:null) != 0) {
