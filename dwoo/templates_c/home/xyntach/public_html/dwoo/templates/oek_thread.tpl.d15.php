...
<div class="thread">
	...
	<?php 
$_fh10_data = (isset($this->scope["posts"]) ? $this->scope["posts"] : null);
$this->globals["foreach"]['postsloop'] = array
(
	"index"		=> 0,
	"first"		=> null,
);
$_fh10_glob =& $this->globals["foreach"]['postsloop'];
if ($this->isArray($_fh10_data) === true)
{
	foreach ($_fh10_data as $this->scope['postkey']=>$this->scope['post'])
	{
		$_fh10_glob["first"] = (string) ($_fh10_glob["index"] === 0);
...
?>
<!-->OP-post link</!--><a href="<?php echo $this->scope["file_path"];?>/src/<?php echo $this->scope["post"]["file"];?>.<?php echo $this->scope["post"]["file_type"];?>" onclick="javascript:expandimg('<?php echo $this->scope["post"]["id"];?>', '<?php echo $this->scope["file_path"];?>/src/<?php echo $this->scope["post"]["file"];?>.<?php echo $this->scope["post"]["file_type"];?>', '<?php echo $this->scope["file_path"];?>/thumb/<?php echo $this->scope["post"]["file"];?>s.<?php echo $this->scope["post"]["file_type"];?>', '<?php echo $this->scope["post"]["image_w"];?>', '<?php echo $this->scope["post"]["image_h"];?>', '<?php echo $this->scope["post"]["thumb_w"];?>', '<?php echo $this->scope["post"]["thumb_h"];?>');return false;"><span id="thumb<?php echo $this->scope["post"]["id"];?>"><img src="<?php echo $this->scope["file_path"];?>/thumb/<?php echo $this->scope["post"]["file"];?>s.<?php echo $this->scope["post"]["file_type"];?>" alt="<?php echo $this->scope["post"]["id"];?>" class="thumb" height="<?php echo $this->scope["post"]["thumb_h"];?>" width="<?php echo $this->scope["post"]["thumb_w"];?>" /></span></a><?php 
}
...
?>
				...
				<?php 
$_fh8_data = (isset($this->scope["posts"]) ? $this->scope["posts"] : null);
if ($this->isArray($_fh8_data) === true)
{
	foreach ($_fh8_data as $this->scope['postkey2']=>$this->scope['post2'])
	{
/* -- foreach start output */
?>
...
?>
				<a href="#top" onclick="javascript:
				<?php 
$_fh9_data = (isset($this->scope["posts"]) ? $this->scope["posts"] : null);
if ($this->isArray($_fh9_data) === true)
{
	foreach ($_fh9_data as $this->scope['postkey2']=>$this->scope['post2'])
	{
/* -- foreach start output */
?>
					<?php if ((isset($this->scope["post2"]["parentid"]) ? $this->scope["post2"]["parentid"]:null) != 0) {
