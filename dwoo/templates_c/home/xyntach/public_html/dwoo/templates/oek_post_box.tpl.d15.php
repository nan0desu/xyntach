...
?>
		...
		<?php 
$_fh6_data = (isset($this->scope["oekposts"]) ? $this->scope["oekposts"] : null);
if ($this->isArray($_fh6_data) === true)
{
	foreach ($_fh6_data as $this->scope['post'])
	{
...
?>
			<?php 
$_fh7_data = (isset($this->scope["board"]["filetypes_allowed"]) ? $this->scope["board"]["filetypes_allowed"]:null);
$this->globals["foreach"]['files'] = array
(
	"iteration"		=> 1,
	"last"		=> null,
	"total"		=> $this->isArray($_fh7_data) ? count($_fh7_data) : 0,
);
$_fh7_glob =& $this->globals["foreach"]['files'];
if ($this->isArray($_fh7_data) === true)
{
	foreach ($_fh7_data as $this->scope['filetype'])
	{
		$_fh7_glob["last"] = (string) ($_fh7_glob["iteration"] === $_fh7_glob["total"]);
/* -- foreach start output */
?>
