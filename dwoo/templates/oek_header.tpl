...
{/if} //-->
{if %KU_WATCHTHREADS}
	[<a href="#" onclick="javascript:showwatchedthreads();return false" title="{t}Watched Threads{/t}">{t}WT{/t}</a>]&nbsp;
{/if}
<!-- [<a href="xmpp://xynta.ch@muc.unstable.nl" target="_top" title="Jabber конференция">{t}Jabber{/t}</a>] //-->
</div>
<div class="navbar">
{if %KU_GENERATEBOARDLIST}
{if %KU_NAME eq "Depressarium"}[ <a href="/b" title="Хуита.ч"> x </a> ]{/if}
...
{/if}
{$title}
<span class="BoardName">{%KU_NAME}</span>
</div>
{$board.includeheader}
<hr />
