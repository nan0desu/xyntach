...
<span class="filesize">{if $post.file_type eq 'mp3'}{t}Аудио: {/t}{else}{t}Файл: {/t}{/if}{if $post.file_type neq 'jpg' && $post.file_type neq 'gif' && $post.file_type neq 'png' && $post.videobox eq ''}<a href="{$file_path}/src/{$post.file}.{$post.file_type}">{else}<a href="{$file_path}/src/{$post.file}.{$post.file_type}" onclick="javascript:expandimg('{$post.id}', '{$file_path}/src/{$post.file}.{$post.file_type}', '{$file_path}/thumb/{$post.file}s.{$post.file_type}', '{$post.image_w}', '{$post.image_h}', '{$post.thumb_w}', '{$post.thumb_h}');return false;">{/if}{if isset($post.id3.comments_html)}{if $post.id3.comments_html.artist.0 neq ''}{$post.id3.comments_html.artist.0} - {if $post.id3.comments_html.title.0 neq ''}{/if}{/if}{if $post.id3.comments_html.title.0 neq ''}{$post.id3.comments_html.title.0}{/if}</a> {else}{$post.file}.{$post.file_type}</a> {/if}({$post.file_size_formatted}{if  $post.file_type neq 'jpg' && $post.file_type neq 'gif' && $post.file_type neq 'png' && $post.file_type neq 'mp3'}{else}, {/if}{if $post.id3.comments_html.bitrate neq 0 || $post.id3.audio.sample_rate neq 0}{if $post.id3.audio.bitrate neq 0}{round($post.id3.audio.bitrate / 1000)}кб/с, {if $post.id3.audio.sample_rate neq 0}{/if}{/if}{if $post.id3.audio.sample_rate neq 0}{$post.id3.audio.sample_rate / 1000}кГц{/if}{/if}{if $post.image_w > 0 && $post.image_h > 0}{$post.image_w}x{$post.image_h}{/if}{if $post.file_original neq '' && $post.file_original neq $post.file}{$post.file_original}.{$post.file_type}{/if}){if $post.id3.playtime_string neq ''} {t}(длительность{/t} — {$post.id3.playtime_string}){/if}</span>
				{if %KU_THUMBMSG}
		...
		<table class="_singlepost">
				...
				<tr>
					...
					<td class="reply" id="reply{$post.id}">
						...
						{if ($post.file neq '' || $post.file_type neq '' ) && (( $post.videobox eq '' && $post.file neq '') && $post.file neq 'removed')}
							<br />
<span class="filesize">{if $post.file_type eq 'mp3'}{t}Аудио: {/t}{else}{t}Файл: {/t}{/if}{if $post.file_type neq 'jpg' && $post.file_type neq 'gif' && $post.file_type neq 'png' && $post.videobox eq ''}<a href="{$file_path}/src/{$post.file}.{$post.file_type}">{else}<a href="{$file_path}/src/{$post.file}.{$post.file_type}" onclick="javascript:expandimg('{$post.id}', '{$file_path}/src/{$post.file}.{$post.file_type}', '{$file_path}/thumb/{$post.file}s.{$post.file_type}', '{$post.image_w}', '{$post.image_h}', '{$post.thumb_w}', '{$post.thumb_h}');return false;">{/if}{if isset($post.id3.comments_html)}{if $post.id3.comments_html.artist.0 neq ''}{$post.id3.comments_html.artist.0} - {if $post.id3.comments_html.title.0 neq ''}{/if}{/if}{if $post.id3.comments_html.title.0 neq ''}{$post.id3.comments_html.title.0}{/if}</a> {else}{$post.file}.{$post.file_type}</a> {/if}({$post.file_size_formatted}{if  $post.file_type neq 'jpg' && $post.file_type neq 'gif' && $post.file_type neq 'png' && $post.file_type neq 'mp3'}{else}, {/if}{if $post.id3.comments_html.bitrate neq 0 || $post.id3.audio.sample_rate neq 0}{if $post.id3.audio.bitrate neq 0}{round($post.id3.audio.bitrate / 1000)}кб/с, {if $post.id3.audio.sample_rate neq 0}{/if}{/if}{if $post.id3.audio.sample_rate neq 0}{$post.id3.audio.sample_rate / 1000}кГц{/if}{/if}{if $post.image_w > 0 && $post.image_h > 0}{$post.image_w}x{$post.image_h}{/if}){if $post.id3.playtime_string neq ''}{t} (длительность{/t} ?— {$post.id3.playtime_string}){/if}</span>
							{if %KU_THUMBMSG}
								<span class="thumbnailmsg"> 
								{if $post.file_type neq 'jpg' && $post.file_type neq 'gif' && $post.file_type neq 'png' && $post.videobox eq ''}
									{t}Для открытия файла нажмите на его значок.{/t}
