			...
			if ($board_class->board['type'] == 0 || $board_class->board['type'] == 2 || $board_class->board['type'] == 3) {
				...
				if ($imagefile_name != '') {
					...
					if (is_array($exists_thread)) {
						exitWithErrorPage(_gettext('Duplicate file entry detected.'), sprintf(_gettext('Было год назад на %sДваче%s.'),'<a href="' . KU_BOARDSPATH . '/' . $board_class->board['name'] . '/res/' . $exists_thread[0] . '.html#' . $exists_thread[1] . '">','</a>'));
					}
						...
						if ($video_id != '' && strpos($video_id, '@') == false && strpos($video_id, '&') == false) {
							...
							if ($results[0] == 0) {
								...
								switch ($video_check[1]) {
									case 404:
										exitWithErrorPage(_gettext('Неправильный URL') .': '. $videourl_start . $video_id);
										break;
									case 303:
								...
								foreach ($results as $line) {
									...
									exitWithErrorPage(sprintf(_gettext('Было год назад на %sДваче%s.'),'<a href="' . KU_BOARDSFOLDER . '/' . $board_class->board['name'] . '/res/' . $real_threadid . '.html#' . $line[1] . '">','</a>'));
								}
							}
						} else {
