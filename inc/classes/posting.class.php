...
class Posting {
	...
	function CheckReplyTime() {
		...
		/* If the time was shorter than the minimum time distance */
			if (time() - $line['timestamp'] <= KU_REPLYDELAY) {
				exitWithErrorPage(_gettext('Пожалуйста, будьте чуть медленнее.'), _gettext('Постить с такой скоростью совершенно недозволительно.'));
			}
		}
	}
	...
	function CheckMessageLength() {
		...
		if (strlen($_POST['message']) > $board_class->board['messagelength']) {
			...
			exitWithErrorPage(sprintf(_gettext('Неоправданно много букв. %d против дозволенных %d. Потрудитесь быть более лаконичным.'), strlen($_POST['message']), $board_class->board['messagelength']));
		}
	}
	...
	function CheckCaptcha() {
		...
		if ($board_class->board['enablecaptcha'] == 1) {
			...
			if ($_SESSION['security_code'] != strtolower($_POST['captcha']) || empty($_SESSION['security_code'])) {
				/* Kill the script, stopping the posting process */
				exitWithErrorPage(_gettext('Плохая, негодная капча. Перепишите.'));
			}
		}
	}
	...
	function CheckBannedHash() {
		...
		if (isset($_FILES['imagefile'])) {
			if ($_FILES['imagefile']['name'] != '') {
				...
				if (count($results) > 0) {
						$bans_class->BanUser($_SERVER['REMOTE_ADDR'], 'SERVER', '1', $results[0]['bantime'], '', 'Плохой, негодный файл. Он не нужен.<br />' . $results[0]['description'], 0, 0, 1);
		...
		if (isset($_POST['replythread'])) {
			if ($_POST['replythread'] != '0') {
				...
				/* If it doesn't... */
					...
					/* Kill the script, stopping the posting process */
					exitWithErrorPage(_gettext('Неверный номер треда.'), _gettext('Возможно, кто-то его просто удалил.'));
				}
			}
		}
	...
	function CheckNotDuplicateSubject($subject) {
		...
		if ($result > 0) {
			exitWithErrorPage(_gettext('Тред с такой темой уже существует.'), _gettext('Пожалуйста, придумайте что-нибудь оригинальное.'));
		}
	}
	...
	function CheckBlacklistedText() {
		...
		foreach ($badlinks as $badlink) {
			if (stripos($_POST['message'], $badlink) !== false) {
				...
				$bans_class->BanUser($_SERVER['REMOTE_ADDR'], 'board.php', 1, 3600, '', _gettext('Ваша ссылка занесена в спамлист. Она не нужна.') . ' (' . $badlink . ')', $_POST['message']);
				exitWithErrorPage(sprintf(_gettext('Обнаружена ссылка ( %s ) из спамлиста.'), $badlink));
			}
		}
	}
}
