...
if (!$cache_loaded) {
	// Database
		...
		$cf['KU_DBHOST']          = '127.0.0.1'; // Database hostname. On SQLite this has no effect.
		$cf['KU_DBDATABASE']      = 'DB_NAME_HERE'; // Database... database. On SQLite this will be the path to your database file. Secure this file.
		$cf['KU_DBUSERNAME']      = 'DB_USERNAME_HERE'; // Database username. On SQLite this has no effect.
		$cf['KU_DBPASSWORD']      = 'DB_PASSWORD_HERE'; // Database password. On SQLite this has no effect.
		$cf['KU_DBPREFIX']        = ''; // Database table prefix
		$cf['KU_DBUSEPERSISTENT'] = false; // Use persistent connection to database
	// Imageboard info
		$cf['KU_NAME']      = 'Хуита.ч'; // The name of your site
		$cf['KU_SLOGAN']    = 'I feel sick.'; // Site slogan, set to nothing to disable its display
		...
		$cf['KU_BANREASON']	= 'Your IP is banned. Use Tor.'; // This is the default ban reason that will automatically fill in the ban reason box
	...
	// Paths and URLs
		// Main installation directory
			...
			$cf['KU_WEBPATH']   = 'http://xynta.ch'; // The path to the index folder of kusaba, without trailing slash. Example: http://www.yoursite.com
			$cf['KU_DOMAIN']    = '.xynta.ch'; // Used in cookies for the domain parameter.  Should be a period and then the top level domain, which will allow the cookies to be set for all subdomains.  For http://www.randomchan.org, the domain would be .randomchan.org; http://zachchan.freehost.com would be zach.freehost.com
	...
	// CSS styles
		$cf['KU_STYLES']        = 'photon:none'; // Styles which are available to be used for the boards, separated by colons, in lower case.  These will be displayed next to [Home] [Manage] if KU_STYLESWIKUHER is set to true
		$cf['KU_DEFAULTSTYLE']  = 'photon'; // If Default is selected in the style list in board options, it will use this style.  Should be lower case
		...
		$cf['KU_TXTSTYLES']        = 'futatxt:buritxt:lighttxt'; // Styles which are available to be used for the boards, separated by colons, in lower case
		...
		$cf['KU_MENUSTYLES']        = 'photon:none'; // Menu styles
		$cf['KU_DEFAULTMENUSTYLE']  = 'photon'; // Default menu style
		$cf['KU_MENUSTYLESWITCHER'] = true; // Whether or not to display the different styles in a clickable switcher in the menu

	// Limitations
		$cf['KU_NEWTHREADDELAY'] = 10; // Minimum time in seconds a user must wait before posting a new thread again
		$cf['KU_REPLYDELAY']     = 5; // Minimum time in seconds a user must wait before posting a reply again
		$cf['KU_LINELENGTH']     = 600; // Used when cutting long post messages on pages and placing the message too long notification
...
#		$cf['KU_THREADS']         = 10; // Number of threads to display on a board page
		$cf['KU_THREADS'] = isset($_POST['board']) && $_POST['board'] == 'z' ? 1 : 10;
		$cf['KU_THREADSTXT']      = 15; // Number of threads to display on a text board front page
		$cf['KU_REPLIES']         = 10; // Number of replies to display on a board page
		...
		$cf['KU_BANMSG']          = '<br /><font color="#0000FF"><b>(USER WAS BUMPED FOR THIS POST)</b></font>'; // The text to add at the end of a post if a ban is placed and "Add ban message" is checked
		...
		$cf['KU_YOUTUBEWIDTH']    = ''; // Width to display embedded YouTube videos
		$cf['KU_YOUTUBEHEIGHT']   = ''; // Height to display embedded YouTube videos
	...
	// Special Tripcodes
		$cf['KU_TRIPS'] = array(); // Special tripcodes which can have a predefined output.  Do not include the initial ! in the output.  Maximum length for the output is 30 characters.  Set to array(); to disable
	...
	// Misc config
		$cf['KU_MODLOGDAYS']        = 666; // Days to keep modlog entries before removing them
		$cf['KU_RANDOMSEED']        = 'YOUR_RANDOM_SEED_HERE'; // Type a bunch of random letters/numbers here, any large amount (35+ characters) will do
	...
	// Language / timezone / encoding
		...
		putenv('TZ=Asia/Dubai'); // The time zone which the server resides in
