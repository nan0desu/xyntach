		...
		if (count($watched_threads) > 0) {
			foreach ($watched_threads as $watched_thread) {
				...
				if ($threadinfo['subject'] != '') {
					$output .= '<span class="watchedthreadstitle">— '.$threadinfo['subject'].'</span> ';
				}
...
// 				}
				...
				if ($numnewreplies > 0) {
					$output .= '— <span class="WatchedThreadsNumberOfUnreadPostsIsOverNull"><a href="' . KU_BOARDSFOLDER . $boardname . '/res/' . $watched_thread['threadid'] . '.html#' . $watched_thread['lastsawreplyid'] . '">' . $numnewreplies . ' новые посты';
// 					if ($numnewreplies != 1) {
// 						$output .= 'ies';
// 					} else {
// 						$output .= 'y';
// 					}
					...
					$output .= '— <span class="WatchedThreadsNumberOfUnreadPostsIsNull">0</span>';
				}

				$output .= ' [<a href="#" onclick="javascript:removefromwatchedthreads(\'' . $watched_thread['threadid'] . '\', \'' . $_GET['board'] . '\');return false;" title="Un-watch">✖</a>]<br />';
			}
		} else {
			$output .= '<span>Избранных тредов нет. Добавьте парочку.</span>';
		}
