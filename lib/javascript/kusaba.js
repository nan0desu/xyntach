...
var Utf8= {
  encode:function(a) {
	...
	c2=a.charCodeAt(i+1);
	c3=a.charCodeAt(i+2);
	b+=String.fromCharCode(((c&15)<<12)|((c2&63)<<6)|(c3&63));i+=3
      }
    }
    return b
  }
...
function removeframes() {
  ...
  for(var i=0;i<a.length;i++)if(a[i].className=="boardlink")a[i].target="_top";document.getElementById("removeframes").innerHTML='Фреймы убраны.';
  return false
}
...
function insert(a) {
  if(!ispage||quick_reply) {
	...
	var c=b.caretPos;c.text=c.text.charAt(c.text.length-1)==" "?a+" ":a
      }
...
function quote(b,a) {
  var v=eval("document."+a+".message");v.value+=(">>"+b+"\r");
  v.focus()
}
...
function togglePassword() {
  ...
  if(d) {
    var e;
    if((a)||(b)||(c))e=d.innerHTML;else e=d.text;
    e=e.toLowerCase();
    var f='<td></td><td></td>';
    if(e==f) {
    ...
    if((a)||(b)||(c))d.innerHTML=f;else d.text=f
  }
  return false
}
...
function toggleOptions(a,b,c) {
  if(document.getElementById('opt'+a)) {
    if(document.getElementById('opt'+a).style.display=='') {
      document.getElementById('opt'+a).style.display='none';document.getElementById('opt'+a).innerHTML=''
    } else {
	...
	d+='All formatting is performed by the user.'
      ...
      document.getElementById('opt'+a).innerHTML=d;document.getElementById('opt'+a).style.display=''
    }
  }
}
...
function set_cookie(a,b,c) {
  if(c) {
    var d=new Date();d.setTime(d.getTime()+(c*24*60*60*1000)); e="; expires="+d.toGMTString()
  }
  else e="";document.cookie=a+"="+b+e+"; path=/"
}
...
function set_stylesheet(a,b,c) {
  ...
  for(var i=0;i<d.length;i++) {
    var f=d[i].getAttribute("rel");var g=d[i].getAttribute("title");if(f.indexOf("style")!=-1&&g){d[i].disabled=true;if(a==g){d[i].disabled=false;e=true}}
  }
...
function delandbanlinks() {
  ...
  if(b) {
    b=b.parentNode;b.innerHTML='[<input type="checkbox" name="fileonly" id="fileonly" value="on" /><label for="fileonly">только файл</label>] <input name="moddelete" onclick="return confirm(_(\'Вы уверены, что хотите удалить эти посты?\'))" value="'+_('Удалить')+'" type="submit" /> <input name="modban" value="'+_('Забанить нахуй')+'" onclick="this.form.action=\''+ku_cgipath+'/manage_page.php?action=bans\';" type="submit" />'
  }
  ...
  for(var i=0;i<c.length;i++) {
	...
	d.innerHTML="";
	var f="";
	new Ajax.Request(ku_boardspath+"/manage_page.php?action=getip&boarddir="+e[1]+"&id="+e[2],{method:"get",onSuccess:function(a){ipaddr=a.responseText.split("=")||"what are you doing, get out, you don't even fit";
	span=document.getElementById(ipaddr[0]);
	span.innerHTML="[IP: "+ipaddr[1]+" <a href=\""+ku_boardspath+"/manage_page.php?action=deletepostsbyip&ip="+ipaddr[1]+"\" title=\""+_('Delete all posts by this IP')+"\">D</a> / <a href=\""+ku_boardspath+"/manage_page.php?action=ipsearch&ip="+ipaddr[1]+"\" title=\""+_('Search for posts with this IP')+"\">S</a>] "+span.innerHTML},onFailure:function(){f="[what are you doing, get out, you don't even fit]";d.innerHTML=f;}});
	...
	f+='id='+e[2]+'&postid='+e[2]+'&cp=y" title="'+_('Child Pornography')+'" onclick="return confirm(_(\'Are you sure that this is child pornography?\'));">CP<\/a>&#93;';c[i].innerHTML=f
      }
    }
  }
}
...
function instantban(c,d) {
  var e=prompt(_('Are you sure you want to permanently ban the poster of this post/thread?\nIf so enter a ban message or click OK to use the default ban reason. To cancel this operation, click "Cancel".'));
  if(e!==null) {
    var f=ku_cgipath+'/manage_page.php?action=bans&banboard='+c+'&banpost='+d+'&instant=y';
    if(e!='') {
      f+='&reason='+e
    }
    new Ajax.Request(f,{method:"get",onSuccess:function(a){var b=a.responseText||"Ban failed!";if(b=="success")alert(_("Ban was sucessful."));else alert(_("Ban failed!"))},onFailure:function(){alert(_("Ban failed!"))}})
  }
  else {
...
function expandthread(c,d) {
  if(document.getElementById('replies'+c+d)) {
    ...
    e.innerHTML=_('Разворачивание треда')+'...<br><br>'+e.innerHTML;
    new Ajax.Request(ku_boardspath+'/expand.php?board='+d+'&threadid='+c,{method:'get',onSuccess:function(a){var b=a.responseText||_("Что-то пошло не так, не эдак. Пустой отклик.");e.innerHTML=b;delandbanlinks();addpreviewevents()},onFailure:function(){alert(_('Что-то пошло не так, не эдак...'))}})
  }
  return false
}
...
function quickreply(a) {
  if(a==0) {
    quick_reply=false;
    document.getElementById('posttypeindicator').innerHTML='новый тред'
  } else {
    qrtoggle();
    quick_reply=true;
    document.getElementById('posttypeindicator').innerHTML='быстрый ответ в тред №'+a+' [<a href="#postbox" onclick="javascript:quickreply(\'0\');" title="Отмена">x</a>]'
  }
...
function getwatchedthreads(c,d) {
  if(document.getElementById('watchedthreadlist')) {
    ...
    e.innerHTML=_('Загрузка избранных тредов...');
    new Ajax.Request(ku_boardspath+'/threadwatch.php?board='+d+'&threadid='+c,{method:'get',onSuccess:function(a){var b=a.responseText||_("Что-то пошло не так, не эдак. Пустой отклик.");e.innerHTML=b},onFailure:function(){alert(_('Что-то пошло не так, не эдак.'))}})}
}

function addtowatchedthreads(c,d) {
  if(document.getElementById('watchedthreadlist')) {
    new Ajax.Request(ku_boardspath+'/threadwatch.php?do=addthread&board='+d+'&threadid='+c,{method:'get',onSuccess:function(a){var b=a.responseText||_("Что-то пошло не так, не эдак. Пустой отклик.");
    alert('Добавлено в «Избранные треды»');
    getwatchedthreads('0',d)},onFailure:function(){alert(_('Что-то пошло не так, не эдак.'))}})
  }
}

function removefromwatchedthreads(c,d) {
  if(document.getElementById('watchedthreadlist')) {
    new Ajax.Request(ku_boardspath+'/threadwatch.php?do=removethread&board='+d+'&threadid='+c,{method:'get',onSuccess:function(a){var b=a.responseText||_("Что-то пошло не так, не эдак. Пустой отклик.");getwatchedthreads('0',d)},onFailure:function(){alert(_('Что-то пошло не так, не эдак.'))}})
  }
}

function hidewatchedthreads() {
  set_cookie('showwatchedthreads','0',30);
  if(document.getElementById('watchedthreads')){document.getElementById('watchedthreads').innerHTML=_('Форма «Избранные треды» будет скрыта после обновления страницы.')+' [<a href="#" onclick="javascript:showwatchedthreads();return false">'+_('Отменить скрытие')+'</a>]'}
}
...
function checkcaptcha(a) {
  if(document.getElementById(a)) {
	...
	alert('Введите капчу. Пожалуйста.');
...
function postpreview(c,d,e,f) {
  if(document.getElementById(c)) {
    new Ajax.Request(ku_boardspath+'/expand.php?preview&board='+d+'&parentid='+e+'&message='+escape(f),{method:'get',onSuccess:function(a){var b=a.responseText||_("Что-то пошло не так, не эдак. Пустой отклик.");document.getElementById(c).innerHTML=b},onFailure:function(){alert(_('Что-то пошло не так, не эдак...'))}})
  }
}
...
function reflinktimeout(e) {
  xynta = this;
  reftimeout = setTimeout(function(){addreflinkpreview(e,xynta)}, 200);
}

function reflinkgone(e) {
  var d="srcElement";
  var f="href";
  if (this[f]) c=this;
  else c=e[d];
  if ($("preview"+c.className)) return;
  clearTimeout(reftimeout);
  o = $("preview"+c.className);
  if (!o) return;
  o.parentNode.removeChild(o);
}

function addreflinkpreview(e,obj) {
  var c;
  var d="srcElement";
  var f="href";
  ...
  z = c.previousSibling;
  if (z.nodeType == 1) {
    if (z.childNodes[1].className == "reflinkpreview") return
  }
  ...
  gp = j.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode
  if (gp.style.left && gp.style.top) {
    xelement = $("prevmod"+gp.id.substr(7));
    xcontent = xelement.childNodes[0].nodeValue.split("|")
    xmod = parseInt(gp.style.left)+parseInt(xcontent[0]);
    ymod = parseInt(gp.style.top)+parseInt(xcontent[1]);
  } else {
    xmod = 0;
    ymod = 0;
  }
  var ftx=document.createTextNode(xmod+"|"+ymod);
  var fffuuu=document.createElement('div');
  fffuuu.style.display = "none";
  fffuuu.id = "prevmod"+g.id.substr(7);
  fffuuu.appendChild(ftx);
  abc = document.createElement('div');
  abc.style.position = "absolute"
  abc.setAttribute('class', 'reflinkpreview');
  abc.setAttribute('className', 'reflinkpreview');
  j.setAttribute('className',c.className);
  if (e.pageX) {
    j.style.left=e.pageX-xmod+20+'px';j.style.top=e.pageY-ymod+'px'
  }
  else {
    j.style.left=e.clientX-xmod+20+'px';j.style.top=e.clientY-ymod+'px'
  }
  j.style.width = innerWidth - parseInt(j.style.left) - xmod - 21 + "px"
  if (j.addEventListener) j.addEventListener("click", delreflinkpreview, false);
  else if (j.attachEvent) j.attachEvent("onmouseout", delreflinkpreview);
  else alert("fffuuuuuuu--");
  Effect.Appear($(j), {duration: "0.15"})
  new Ajax.Request(
    ku_boardspath+'/read.php?b='+ainfo[1]+'&t='+ainfo[2]+'&p='+ainfo[3]+'&single',
    {
      method:'get',
...
function addpreviewevents() {
  ...
  for(var i=0;i<a.length;i++) {
    b=a[i];
    if (b.className && !b.onmouseover) {
      if (b.className.substr(0,4)=="ref|") {
	if (b.addEventListener) {
	  b.addEventListener("mouseover",reflinktimeout,false);
	  b.addEventListener("mouseout",reflinkgone,false);
	} else if (b.attachEvent) {
	  b.attachEvent("onmouseover",reflinktimeout)
	  b.attachEvent("onmouseout",reflinkgone)
	} else alert("Function addpreviewevents failed.")
      }
    }
  }
}
...
function keypress(e) {
  ...
  if(e.altKey) {
	...
	if(a.indexOf('.html')==-1||a.indexOf('board.html')!=-1) {
	  ...
	  var b=a.substr((a.lastIndexOf('/')+1));b=(+b.substr(0,b.indexOf('.html')));
	  var c=a.substr(0,a.lastIndexOf('/')+1)
	}
	if(b==0) {
	  var d=c
...
window.onload=function(e) {
  ...
  if(document.getElementById('watchedthreads')) {
    var g=new Draggable('watchedthreads',{handle:'watchedthreadsdraghandle',onEnd:function(){watchedthreadsdragend()}});
    var h=new Resizeable('watchedthreads',{resize:function(){watchedthreadsresizeend()}});
...
if(style_cookie) {
  var cookie=getCookie(style_cookie);var title=cookie?cookie:get_preferred_stylesheet();if(title!=get_active_stylesheet())set_stylesheet(title)
}
