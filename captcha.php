...
//500 60 0.65
	function CaptchaSecurityImages($width="",$height="",$characters="",$font) {
		...
		function InitRandColors($range_min, $range_max, &$out) {

		    $out["r"] = rand($range_min, $range_max);
		    $out["g"] = rand($range_min, $range_max);
		    $out["b"] = rand($range_min, $range_max);

		    return $out;
		};

		InitRandColors(238, 238, $out);
		$background_color = imagecolorallocate($image, $out["r"], $out["g"], $out["b"]);

		InitRandColors(0, 50, $out);
		$text_color = imagecolorallocate($image, $out["r"], $out["g"], $out["b"]);

		InitRandColors(70, 100, $out);
		$noise_color = imagecolorallocate($image, $out["r"], $out["g"], $out["b"]);
...
$font = KU_ROOTDIR . 'lib/fonts/segoescb.ttf';
